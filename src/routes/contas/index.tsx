...
export const Route = createFileRoute("/_layout/contas")({
	validateSearch: search => ({ order: search.order as string }),
	component: () => {
		...
		const { order: orderFilter } = Route.useSearch();
		...
		const pendingPaymentsCount = getFilteredCharges(
			"pendente",
			orderFilter,
		).length;
		const overduePaymentsCount = getFilteredCharges(
			"vencido",
			orderFilter,
		).length;
				...
				>
					<div className="mb-5 text-left">
						...
						<p className="text-gray-600 text-sm md:text-base">
							...
							{orderFilter && (
								<span className="font-medium"> - Pedido #{orderFilter}</span>
							)}
						</p>
					</div>
					...
					>
						...
						>
							<ChargesList
								filteredCharges={getFilteredCharges("all", orderFilter)}
						...
						>
							<ChargesList
								filteredCharges={getFilteredCharges("pago", orderFilter)}
								handleOpenModal={handleOpenModal}
							/>
						</TabsContent>
