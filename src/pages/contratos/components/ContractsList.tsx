				...
				{orderFilter && (
					<div className='flex items-center justify-between border-blue-100 border-b bg-blue-50 p-4'>
						<div>
							...
							<p className='text-blue-600 text-sm'>{filteredContracts.length} contrato(s) encontrado(s)</p>
						</div>
						<Button
							variant='outline'
							size='sm'
					...
					<Table>
						<TableHeader>
							<TableRow>
								...
								<TableHead className='hidden whitespace-nowrap md:table-cell'>Tipo</TableHead>
								<TableHead className='whitespace-nowrap'>Status</TableHead>
								<TableHead className='hidden whitespace-nowrap md:table-cell'>Pedido</TableHead>
								<TableHead className='hidden whitespace-nowrap md:table-cell'>Valor</TableHead>
						...
						<TableBody>
							{filteredContracts.map((contract) => {
								...
								return (
									...
									>
										<TableCell className='whitespace-nowrap font-medium'>{formatDate(contract.startDate)}</TableCell>
										<TableCell className='hidden md:table-cell'>{getContractTypeDisplay(contract.type)}</TableCell>
										<TableCell className='whitespace-nowrap'>
											<StatusBadge status={contract.status} />
										</TableCell>
										<TableCell className='hidden md:table-cell'>
											{contract.orderId && <span className='font-medium text-sm'>#{contract.orderId}</span>}
										</TableCell>
										<TableCell className='hidden md:table-cell'>
											{associatedOrder && <span>{formatCurrency(associatedOrder.price)}</span>}
										</TableCell>
