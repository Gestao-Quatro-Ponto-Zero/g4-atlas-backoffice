		...
		<Card>
			...
			<CardContent>
				<div className='space-y-0 divide-y'>
					<ProfileItem label='Nome' value={userData?.name} onClick={() => setIsNameDialogOpen(true)} />
					<ProfileItem label='E-mail' value={userData?.email} onClick={() => setIsEmailDialogOpen(true)} />
					<ProfileItem label='Telefone' value={userData?.phone} onClick={() => setIsPhoneDialogOpen(true)} />
				</div>
			</CardContent>

			<NameDialog isOpen={isNameDialogOpen} onOpenChange={setIsNameDialogOpen} initialName={userData?.name || ''} />
			...
			{verifyingField && (
				<OtpDialog
					...
					contactValue={verifyingField === 'email' ? userData?.email || '' : userData?.phone || ''}
